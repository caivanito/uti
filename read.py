...
def ReadXLSX(path, file):
    full_file = path + file
    values_datos_generales = {}
    values_monitoreo = {}
    wb = load_workbook(full_file, data_only=True)

    # sh_general = 'General'
    sh_monitoreo = 'Monitoreo'
    sh_estadistica = 'Estadística nueva'
    sh = getSheet(sh_estadistica, wb)
    try:
        if sh is not None:
            rows, fin_columns = getEstadistica_Context()  # HOJA ESTADISTICA
            aux = ''
            for row in sh.iter_rows(rows, rows, sh.min_column, fin_columns):
                for cell in row:
                    context_generic = {}
